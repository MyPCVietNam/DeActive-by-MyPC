...
- Thay kiểm tra quyền Admin bằng WindowsPrincipal; không còn phụ thuộc `net session` / dịch vụ Server (LanmanServer).
- Sửa lỗi `%errorlevel%` bị CMD expand sớm trong block `if (...)`, có thể làm báo sai UAC thất bại.
- Đơn giản hóa nâng quyền: PowerShell `Start-Process` chạy trực tiếp file `.cmd` với verb `RunAs`, tránh chuỗi quote lồng `cmd /c` dễ lỗi.
- Thêm `EasyActive-Diagnose.cmd` để kiểm tra launcher/PowerShell/quyền Admin và luôn `pause`, không thay đổi hệ thống.
...
- **Đổi tên tool: `DeActive by MyPC` → `EasyActive by MyPC`.** Cái tên mới nói đúng mục đích hơn — *kích hoạt bản quyền dễ dàng* — thay vì dễ bị hiểu nhầm sang "gỡ kích hoạt".
- Đổi tên file: `EasyActive-Engine.ps1`, `EasyActive-Menu.cmd`, `EasyActive-by-MyPC.bat`.
- **Đổi thư mục dữ liệu:** log/báo cáo/backup nay nằm ở `C:\ProgramData\EasyActiveByMyPC\` (trước là `...\DeActiveByMyPC\`). *Lưu ý:* dữ liệu cũ vẫn nằm ở thư mục cũ, không tự chuyển sang — có thể xóa thủ công nếu không cần.
- Chức năng giữ nguyên như v1.8.3.
...
- **Đổi tên/branding cho bớt nhạy cảm:** File `Clean-MAS-Activation.ps1` → `EasyActive-Engine.ps1`, `Clean-MAS-Activation.cmd` → `EasyActive-Menu.cmd`. Thư mục dữ liệu `C:\ProgramData\LegitActivationCleaner` → `C:\ProgramData\EasyActiveByMyPC` (log, report, backup, tên file report đều theo tên mới). File `EasyActive-by-MyPC.bat` giữ nguyên là điểm chạy chính.
...
EasyActive by MyPC hỗ trợ dọn trạng thái kích hoạt cũ của Windows và Microsoft Office để người dùng có thể chuyển sang key bản quyền chính hãng hoặc đăng nhập tài khoản Microsoft/Microsoft 365 hợp lệ.
...
1. Giải nén tool.
2. Chuột phải `EasyActive-by-MyPC.bat`.
...
- `EasyActive-by-MyPC.bat` - launcher thương hiệu public.
- `EasyActive-Menu.cmd` - menu chạy tool, hỗ trợ tiếng Việt/English.
- `EasyActive-Engine.ps1` - script PowerShell chính.
